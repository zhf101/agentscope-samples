...
import { RoadMap, RoadMapDataProps, RoadMapType } from "@/types/roadmap";
...
const SharePage: React.FC = () => {
  ...
  const mapRoadmapStatus = (status: RoadMap["decomposed_tasks"][number]["status"]) => {
    if (status === "Done") return RoadMapType.DONE;
    if (status === "In-process") return RoadMapType.IN_PROGRESS;
    return RoadMapType.TODO;
  };
  const toRoadMapData = (
    roadmap?: RoadMap | null,
  ): RoadMapDataProps | undefined => {
    if (!roadmap || !Array.isArray(roadmap.decomposed_tasks)) {
      return undefined;
    }
    return {
      subtasks: roadmap.decomposed_tasks.map((item) => ({
        description:
          item.subtask_specification?.subtask_description ||
          item.explanation ||
          "Untitled task",
        state: mapRoadmapStatus(item.status),
      })),
    };
  };
  const startTimer = () => {
        ...
        >
          ...
          <div className={styles.tabContent}>
            {activeTab === "workspace" ? (
              ...
              <Roadmap
                data={toRoadMapData(roadmapData)}
                editable={false}
