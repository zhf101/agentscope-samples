...
This router lets the sandbox dynamically:
1. register MCP servers from config,
2. discover each server's tools,
3. proxy tool calls to the right server session.
"""
    ...
    try:
        ...
        for server in new_servers:
            if server.name in _MCP_SERVERS:
                if not overwrite:
                    # Keep existing live session unless caller explicitly asks
                    # for replacement.
                    continue
                # Cleanup old server
        ...
        for server_name, server in _MCP_SERVERS.items():
            ...
            for tool in tools:
                ...
                if name in server_tools:
                    ...
                    # Convert MCP tool schema into the function-tool schema
                    # expected by caller components.
                    json_schema = {
                        "type": "function",
                        "function": {
                            "name": tool.name,
...
async def call_tool(
    ...
    try:
        ...
        # Tool names are unique per server map here; search and dispatch.
...
async def cleanup_servers() -> None:
    ...
    # Reverse order mirrors stack-like initialization and is safer for
...
async def startup_event():
    """Load static MCP configs at boot and pre-initialize server sessions."""
