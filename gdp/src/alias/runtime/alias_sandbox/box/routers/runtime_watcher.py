# -*- coding: utf-8 -*-
"""Git watcher utilities exposed through HTTP.

These endpoints provide basic VCS operations for runtime tooling:
- commit current workspace changes,
- generate file-level unified diffs,
- list commit history with diff payloads.
"""
...
def initialize_git_user(repo):
    """Ensure repo has fallback identity so automated commits can succeed."""
...
async def generate_diff(
    ...
    """
    Generate unified diffs for uncommitted changes or between two commits.
    """
    try:
        repo_path = "."
        repo = git.Repo(repo_path)
        ...
        for diff in diff_index:
            ...
            # Unified diff format is convenient for LLM and human review tools.
            diff_text = "\n".join(
...
async def git_logs():
    """
    Return commit history, each with metadata and parent diff.
    """
    try:
        repo = git.Repo(".")
        repo = initialize_git_user(repo)
