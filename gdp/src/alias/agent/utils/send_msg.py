...
def _extract_thinking_content(msg: Msg) -> str:
    """Extract plain text from thinking blocks."""
...
def _create_assistant_message(
    ...
    if msg_type == MessageType.CLARIFICATION:
        ...
        if isinstance(content_to_send, Msg):
            if msg_type in {MessageType.THOUGHT, MessageType.SUB_THOUGHT}:
                content = _extract_thinking_content(content_to_send)
            else:
...
def _determine_message_type(content_to_send: Union[str, Msg]) -> MessageType:
    ...
    if (
        ...
        content_to_send,
        Msg,
    ) and content_to_send.has_content_blocks("thinking"):
        if content_to_send.name == DEFAULT_PLANNER_NAME:
            return MessageType.THOUGHT
        else:
            return MessageType.SUB_THOUGHT
